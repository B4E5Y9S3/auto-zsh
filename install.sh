...
install_oh_my_zsh() {
  if [ ! -d ~/.oh-my-zsh ]; then
    ...
    (bash -c 'sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" --unattended') &>/dev/null &
    spin
...
install_zsh_plugins() {
  ...
  spin
  git clone https://github.com/zsh-users/zsh-completions.git "$ZSH_CUSTOM/plugins/zsh-completions" &>/dev/null &
  spin
}
